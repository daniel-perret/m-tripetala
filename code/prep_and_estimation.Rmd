...
## plots
wy.p2a.plotlink <- read.csv("/Users/DanielPerret/Box/01. daniel.perret Workspace/PROJECTS/spruce-fir-demography/data/wy_p2alink.csv",header=T,stringsAsFactors=F) %>%
...
all.fia$PLOT <- all.fia$PLOT %>% 
  ...
  mutate(REMPER = ifelse(is.na(REMPER),
                         ...
                         REMPER))
## trees
wy.p2a.treelink <- read.csv("/Users/DanielPerret/Box/01. daniel.perret Workspace/PROJECTS/spruce-fir-demography/data/wy_p2alink_tree.csv",header=T,stringsAsFactors=F) %>% 
...
clim.ann <- clim.ann %>% filter(ID1 %in% fuzz.coords$ID1)
